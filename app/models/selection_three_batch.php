...
/**
 * SelectionThreeAssignment
 *
 * @author Andhika Nugraha <andhika.nugraha@gmail.com>
 * @package applicant
 */
class SelectionThreeBatch extends HeliumRecord {
	public $id;
	...
	public static function get_national_announcement_date() {
		return new HeliumDateTime(Helium::conf('selection_two_announcement_date'));
	}
	
	public static function get_national_selection_date() {
		return new HeliumDateTime(Helium::conf('selection_three_date'));
	}
	
	public function generate_local_batch_number() {
		$db = Helium::db();
		$query = $db->prepare("SELECT MAX(local_batch_number) FROM selection_three_batches WHERE chapter_id='%d' ORDER BY local_batch_number DESC", $this->chapter_id);
	...
	public function get_participant_count() {
		return Helium::db()->get_var('SELECT COUNT(*) FROM participants WHERE selection_three_batch_id=' . $this->id);
	}
}
